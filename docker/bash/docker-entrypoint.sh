...
if [ "$originalArgOne" = 'mongod' ]; then
	...
	if [ -n "$shouldPerformInitdb" ]; then
		...
		#_mongod_hack_ensure_arg_val --bind_ip_all "${mongodHackedArgs[@]}"
		...
		"${mongodHackedArgs[@]}" --bind_ip_all --keyFile /data/file.key --auth --replSet rs0 --fork
...
#exec "$@"
#echo "${mongodHackedArgs[@]} --bind_ip_all --auth --replSet rs0 --fork"
#"${mongodHackedArgs[@]} --bind_ip_all --auth --replSet rs0 --fork"
mongod --port 27017 --tlsMode disabled --logpath /proc/1/fd/1 --logappend --pidfilepath /tmp/docker-entrypoint-temp-mongod.pid  --bind_ip_all --keyFile /data/file.key --auth --replSet rs0
